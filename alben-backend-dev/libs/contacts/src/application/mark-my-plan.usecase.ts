...
import {
  MarkMyPlanRequestDto,
  MarkMyPlanTypeEnum,
} from '../ui/dtos/mark-my-plan.dto';

@Injectable()
export class MarkMyPlanUseCase {
  constructor(
  ...
  async execute(
    ...
    if (noteIds.length > 0) {
      await this.noteRepo.update(noteIds, { isMyPlan: true });
    }

    if (callIds.length > 0) {
      await this.userProductContactRepo.update(callIds, { isMyPlan: true });
    }

    return {
      success: true,
      code: 'MY_PLAN_UPDATED',
      message: 'Items marked as My Plan successfully.',
      data: null,
    };
  }
}
